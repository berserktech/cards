...
AFRAME.registerComponent("collider-check", {
  // dependencies: ["raycaster-mouse"],

  init: function() {
    var self = this;
    this.position = Object.assign({}, this.el.object3D.position);
  ...
  tick: function() {
    if (this.el.object3D.position.x != this.position.x) {
      this.el.object3D.position.x +=
        (this.el.object3D.position.x - this.position.x) / 100;
      this.el.object3D.position.y +=
        (this.el.object3D.position.y - this.position.y) / 100;
    }
  }
});
...
AFRAME.registerComponent("raycaster-mouse", {
  ...
  init: function() {
    ...
    this.circleRads = { R: 12, a: 0, b: Math.PI / 2 };
    ...
    window.addEventListener("wheel", event => {
      ...
      this.circleRads.a += event.deltaX / 1000;
      this.circleRads.b += event.deltaY / 1000;
  ...
  setPosition: function(event) {
    ...
    var clientX = event.changedTouches
      ? event.changedTouches[0].clientX
      : event.clientX;
    var clientY = event.changedTouches
      ? event.changedTouches[0].clientY
      : event.clientY;

    this.circleRads.a += (this.lastClientX - clientX) / 100;
    this.circleRads.b += -(this.lastClientY - clientY) / 100;
    this.updateCameraPosition();
  ...
  tick: function(a, b) {
    if (this.rotate) {
      if (
        this.justLoaded ||
        (!this.mouseDown &&
          this.mouseUpTime.getTime() + 3000 < new Date().getTime())
      ) {
        this.circleRads.a += 1 / 1000 / 2;
        this.updateCameraPosition();
      }
    }
  },
  ...
  onDocumentMouseMove: function(event) {
    ...
    if (this.mouseDown || event.changedTouches) {
  ...
  checkIntersections: function() {
    ...
    rawIntersections = this.raycaster.intersectObjects(
      this.objects,
      data.recursive
    );
    ...
    console.log("intersectedEls", intersectedEls);
    ...
    for (i = 0; i < newIntersectedEls.length; i++) {
      if (emitedUuid.indexOf(newIntersectedEls[i].object3D.uuid) === -1) {
        console.log(
          "Emit raycaster-intersected on:",
          newIntersectedEls[i].object3D.uuid
        );
